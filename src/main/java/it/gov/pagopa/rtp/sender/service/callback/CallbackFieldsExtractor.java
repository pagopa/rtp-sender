...
/**
 * Utility component responsible for extracting relevant fields from the SEPA
 * callback JSON payload.
 *
 * <p>
 * This class is used to extract domain-specific values such as
 * {@link TransactionStatus}
 * and {@link ResourceID} from nested structures inside a {@link JsonNode},
 * following the
...
public class CallbackFieldsExtractor {
  ...
  /**
   * Extracts a stream of {@link TransactionStatus} values from the provided SEPA
   * callback JSON.
   *
   * <p>
   * This method navigates the structure of the JSON tree under:
   *
   * <pre>
   ...
   *
   * <p>
   * Each textual status is trimmed and mapped to a {@link TransactionStatus}
   * enum. If the status
   * is unknown, it is logged and mapped to {@link TransactionStatus#ERROR}. If
   * the structure is
   ...
   *
   * <p>
   * This method navigates the following JSON path to locate the message ID:
   *
   * <pre>
   *   AsynchronousSepaRequestToPayResponse
   *     └── Document
   ...
   *
   * <p>
   * The extracted message ID is trimmed, validated, and converted into a
   * {@link java.util.UUID}, then
   * wrapped in a {@link ResourceID}. If the field is missing or malformed, an
   * {@link
